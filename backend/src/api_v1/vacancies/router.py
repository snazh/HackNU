...
async def get_all_vacancies(
        session: AsyncSession = Depends(get_async_session),
        service: VacancyService = Depends(get_vacancy_service)):
    vacancies = await service.get_all(session=session)
...
async def change_status(    vacancy_id:int,
    ...
    await service.change_status(vacancy_id=vacancy_id,current_user_payload=user, new_status=new_status, session=session)
    return {"status":"Success", "msg":"Vacancy status changed"}


@router.post("/{vacancy_id}/apply", status_code=status.HTTP_201_CREATED)
async def apply_to_vacancy(application_data: VacancyApplicationCreateSchema,
                           user: UserPayloadSchema = Depends(get_current_user),
                           _: None = require_role(UserRole.admin, UserRole.user),
                           session: AsyncSession = Depends(get_async_session),
                           service: VacancyApplicationService = Depends(get_vacancy_service),
                           vacancy_service: VacancyService = Depends(get_vacancy_application_service)):
    vacancy = await vacancy_service.get_one_by(field="id", value=application_data.vacancy_id, session=session)
    if vacancy is None:
        raise ItemNotFoundError(item="vacancy", attr="id", value=application_data.vacancy_id)
    if vacancy.status != VacancyStatus.OPEN:
        raise VacancyClosedError()


    existing_application = await service.get_one_by(
        session=session,
        field="user_id",
        value=application_data.user_id
    )
    if existing_application:
        raise ItemAlreadyExistsError(item="Application", attr="user_id", value=application_data.user_id)
    application_data.user_id = user.user_id
    applicant_resume = await service.apply(application_data=application_data, session=session)
    questions = await generate_follow_up_questions(resume=applicant_resume, vacancy=vacancy)
    return {"status": "Success", "msg": "Application sent. Questions", "data": questions}
